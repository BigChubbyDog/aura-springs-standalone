...
export default function EmailCapture({ variant = 'popup', delay = 5000, onClose }: EmailCaptureProps) {
  ...
  // Show popup after delay - FIXED: Use useEffect instead of useState
  useEffect(() => {
    if (variant === 'popup' && !hasInteracted) {
      const timer = setTimeout(() => {
        // FIXED: Check if we're in the browser before accessing localStorage
  ...
  // Exit intent detection - FIXED: Use useEffect
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // FIXED: Check if we're in the browser before using localStorage
      if (typeof window !== 'undefined') {
        // Mark as completed
